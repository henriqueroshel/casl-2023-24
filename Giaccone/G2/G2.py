...
def simulation(X, confidence):
    ...
    ci = t.interval(confidence, n-1, avg, std/(n**.5))
    acc = accuracy(ci, avg)
    return ci, acc
...
def main():
    ...
    # Generation of n experiments uniformly distributed
    X = np.random.uniform(MIN_, MAX_, N_EXPERIMENTS[-1])
    for i, n in enumerate(N_EXPERIMENTS):
        # extract the generated values for the current n
        X_n = X[:n] 
        ci, acc = simulation(X_n, confidence)
        ci_lower[i] = ci[0]
        ci_upper[i] = ci[1]
        accs[i] = acc
